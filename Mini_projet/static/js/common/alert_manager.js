...
// Ensure gettext is defined globally
if (typeof gettext === 'undefined') {
    window.gettext = function (msg) { return msg; };
}
...
class SweetModalManager {
    ...
    createModal({
        ...
        this.modal.className = 'hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-[9999] justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full';
        ...
        this.modal.innerHTML = `
                <div class="relative p-4 w-full max-w-md max-h-full z-[10000]">
        ...
        this.modalInstance = new Modal(this.modal, {
            backdrop: 'dynamic',
            backdropClasses: 'bg-gray-900/50 fixed inset-0 z-[9998]',
            closable: false,
            onHide: () => {
                if (!this.isClosing) {
                    this.isClosing = true;
